                          ...
                          n = NULL, power = NULL, verbose = TRUE)
{
  ...
  if(arcsin.trans) {
    ...
    if(verbose) {
      cat(" Approach: Arcsine Transformation (h = ", round(h, 4), ") \n", sep = "")
    }
  } else {
    var.num <- p * (1 - p)
    h <- switch(alternative,
                          ...
                          n = NULL, power = NULL, verbose = TRUE)
{
  ...
  if (r < r0 & alternative == "greater")
    stop("r < r0 but alternative  = 'greater' than r0",
         call. = FALSE)
  if (r > r0 & alternative == "less")
    stop("r > r0 but alternative  = 'less' than r0",
         call. = FALSE)
  if (is.null(n) & is.null(power))
    stop("`n` and `power` cannot be `NULL` at the same time",
         call. = FALSE)
...
pwrss.t.regression <- pwrss.t.reg <- function (beta1 = 0.25, beta0 = 0, margin = 0,
  ...
  if(alternative == "equivalent") {
    ...
    if(beta1 < beta0) stop("alternative = 'greater' but beta1 < beta0", call. = FALSE)
...
.chisq.fun <- function(p1, p0 = NULL) {
  ...
  if(is.null(p0)) {
    ...
    if(isFALSE(all(dim(p1) == dim(p0)))) stop("dimensions should match `p1`", call. = FALSE)
  }
...
.plot.t.t1t2 <- function(ncp, df = Inf, alpha, alternative,
  ...
  if(alternative == "equivalent") {
    ...
    .paint.t.dist(ncp = ncp, df = df, xlim = c(talpha[1], talpha[2]), type = 2)
    
    power.left <- .paint.t.dist(ncp = ncp, df = df, xlim = c(talpha[1], min(xlim)), type = 3)
    power.right <- .paint.t.dist(ncp = ncp, df = df, xlim = c(talpha[2], max(xlim)), type = 3)
